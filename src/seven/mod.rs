use crate::{
    grid::Grid,
    seven::recursive::{recurse_once, recurse_up_aided},
};
...
pub fn parse_input(s: &str) {
    ...
    recurse_once(&mut grid, &mut state, x, y);
...
pub fn recurse_up(s: &str) {
    let mut grid = Grid::new(s);

    let mut state = RecurseState::new(10);

    let last_col = grid.cols - 1;

    let row = grid.get_row(last_col);

    for (x, c) in row.iter().enumerate() {
        if *c == '|' {
            println!("starting at: x:{} y: {}", x, last_col);

            recurse_up_aided(&mut grid, &mut state, x, last_col, false);
...
...............";
    ...
    fn test_recurse_up() {
        let s =
            read_to_string("src/seven/annotated.txt").expect("expected src/seven/annotated.txt");
